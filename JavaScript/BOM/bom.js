...
function GetWindowSize() {
    ...
    document.getElementById("window-size").innerHTML = `Размер окна браузера: ${window_with}x${window_height}`;
}

function GetScreenProperties() {
    let screen_with = window.screen.width;
    let screen_height = window.screen.height;
    document.getElementById("screen-resolution").innerHTML = `${screen_with}x${screen_height}`;
